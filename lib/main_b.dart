...
class _FireStoreAppState extends State<FireStoreApp> {
  ...
  void refreshPage() {
    //you can also pass value back here
    if (mounted) {
      setState(() {});
    }
  }
      ...
      onKeyEvent: (event) {
        ...
        if (DateTime.now().millisecondsSinceEpoch - lastKeyTime > 1000) {
          ...
          if (bluetoothKey == 'Arrow Left') {
            ...
            FlashcardOperation.upperClick(refreshPage);
          } else if (bluetoothKey == 'Arrow Down' ||
              bluetoothKey == 'Game Button A') {
            FlashcardOperation.lowerClick(refreshPage);
          }
        }
        lastKeyTime = DateTime.now().millisecondsSinceEpoch;
      },
...
class FlashcardOperation {
  static upperClick(refreshPage) {
    ...
    if (displayState == "none") {
      readLatestQuestion(refreshPage);
    }
    if (displayState == "upper") {
      readAnswers(refreshPage);
    }
    if (displayState == "lower") {
      if (audioState['promptMode'] == 'flashcard') {
        playLength += 100;
      }
      final text = localFlashcards[questionID]['text'];
      ...
      displayState = "none";
      flashPanes = ['', '', ''];
      lowerClick(refreshPage);
    }

    Timer(const Duration(seconds: 1), () => refreshPage());
  }

  static lowerClick(refreshPage) {
    Wakelock.enable();
    wakeTimer.reset();
    final currentTime = DateTime.now().millisecondsSinceEpoch;
  ...
  static readLatestQuestion(refreshPage) async {
    questionID = '';
    while (questionID == '') {
      if (relations['parents_empty'] == true &&
          relations['children_empty'] == true) {
  ...
  static readAnswers(refreshPage) async {
